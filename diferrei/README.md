...
Diogo was tasked with implementing the algorithmic sorting engines, the internal stack operation mechanics`(ra, rb, rr, rra, rrb, rrr, sa, sb, ss, pa, pb)`, and the data-tracking benchmark system. Specifically, he developed the `sort_simple` (with its `sort_three` and `sort_two` optimizations),`sort_medium` chunk-sorting, and the binary bit-shifting `sort_complex` Radix sort. He also built the core memory-safe argument utility pipeline `(ft_split and its helpers)` to process string inputs, as well as the instruction counting routers `(count_op)` used to track and audit individual operation metrics.
### Rodrpere;

Rodrpere was tasked with the main function, header, atol and correcting early errors, as well doing the Makefile and Check Disorder, so mostly, the early development of basics and necessary functions. Also made a command to help finding errors such as Radix Sort 2-bit problem. 
